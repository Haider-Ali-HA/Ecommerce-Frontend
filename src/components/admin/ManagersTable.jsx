 ...
 */
const ManagersTable = ({ managers = [], onDelete, deletingId }) => {
  return (
    <div className="overflow-x-auto bg-base-100 rounded shadow">
      <table className="table">
        ...
        <tbody>
          {managers.length === 0 ? (
            <tr>
              <td colSpan={5} className="text-center py-8 text-text-accent">
                No managers found.
              </td>
            </tr>
          ) : (
            managers.map((m, idx) => {
              ...
              return (
                <tr key={m._id} className={rowClass}>
                  ...
                  <td>{m.phone}</td>
                  <td>{m.isVerified ? 
                  (<span className="text-success bg-success/40 px-4 py-1 rounded-full">Verified</span>)
                  
                  : 
                  (<span className="text-error bg-error/40 px-4 py-1 rounded-full">Not Verified</span>)
                  }</td>
                  <td>
                    <div className="flex gap-2 justify-end">
                      <Link
                        to={`/admin/managers/update?id=${m._id}`}
                        className={`px-3 py-2 rounded flex items-center justify-center ${isDark ? 'bg-secondary/80 text-text-primary hover:bg-secondary' : 'bg-primary text-text-secondary hover:bg-primary/80'}`}
                      >
                        <Edit className="h-4 w-4 text-info" />
                      </Link>
                      <LoadingButton
                        className={`px-3 py-2 rounded ${isDark ? 'bg-secondary/80 text-text-primary hover:bg-secondary': 'bg-primary text-text-secondary hover:bg-primary/80'}`}
